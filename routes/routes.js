...
router.get('/edit/:id', async (req, res, next) => {
    try {
      ...
      if (!product) {
        // If product cannot be found, redirect to main page
        return res.redirect('/');
      }
  
      // If product is found, return the product edit page (render view or return JSON)
      res.render('edit_product', { title: "Edit Product", product }); // Suppose there is a view nameed 'edit_product'
      
    } catch (err) {
      // If there is error, pass the error to Express's error handling middleware
      next(err);
    }
  });
